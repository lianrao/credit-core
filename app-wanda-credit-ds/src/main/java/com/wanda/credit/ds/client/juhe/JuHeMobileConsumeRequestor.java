 ...
 * @author liunan
 */
@DataSourceClass(bindingDataSourceId="ds_juhe_mobile_consume")
public class JuHeMobileConsumeRequestor extends BaseJuheDSRequestor
implements IDataSourceRequestor {
	private final  Logger logger = LoggerFactory.getLogger(JuHeMobileConsumeRequestor.class);
	@Autowired
	...
	public Map<String, Object> request(String trade_id, DataSource ds){
		...
		try{
			String url = propertyEngine.readById("ds_juhe_consume_url");
			...
			req_params.put("key", propertyEngine.readById("ds_juhe_consume_key"));
			...
			if(isSuccess(rspData)){
			...
			}else if("226104".equals(rspData.getString("error_code"))){
				...
				return rets;
			}else if("226102".equals(rspData.getString("error_code"))){
				logger.info("{} 无法验证" , prefix);
				resource_tag = Conts.TAG_SYS_ERROR;
			...
			rets.put(Conts.KEY_RET_TAG, new String[]{resource_tag});
			rets.put(Conts.KEY_RET_DATA, JsonFilter.getJsonKeys(result, "code"));
