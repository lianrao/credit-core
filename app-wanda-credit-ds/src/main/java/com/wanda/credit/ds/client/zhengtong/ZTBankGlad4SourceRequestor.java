...
import com.wanda.credit.ds.client.jixin.JiXinAuthenBankCard4DataSourceRequestor;
 ...
 * 描述: 银行卡鉴权
 */
@DataSourceClass(bindingDataSourceId = "ds_zhengt_bank4")
public class ZTBankGlad4SourceRequestor extends BaseZTDataSourceRequestor
		implements IDataSourceRequestor {
	private final Logger logger = LoggerFactory.getLogger(ZTBankGlad4SourceRequestor.class);
	@Autowired
	public IPropertyEngine propertyEngine;
	@Autowired
    protected IAllAuthCardService allAuthCardService;
	@Autowired
	private JiXinAuthenBankCard4DataSourceRequestor jixinCard4;
	@Override
	public Map<String, Object> request(String trade_id, DataSource ds) {
		final String prefix = trade_id + " " + Conts.KEY_SYS_AGENT_HEADER;
		logger.info("{} 政通银行卡四要素服务请求开始...", prefix);
		...
		try {
			...
			logger.info("{} 政通银行卡四要素服务加密成功!", prefix);
			...
			if (StringUtils.isNotEmpty(CardNoValidator.validate(cardNo))) {
				...
				logger.info("{} 政通银行卡四要素服务请求结束,返回信息:{}", prefix, res);
...
//				zTFace251Service.add(ztFRes);
				...
				if("2000".equals(ztFRes.getRespcd())){
					...
					if(ztFRes.getRespinfo().contains("请求频繁") || ztFRes.getRespinfo().contains("交易次数超限")){
						...
						ds.setId("ds_jixin_AuthenBankCard4");
			        	rets = jixinCard4.request(trade_id, ds);
			        	return rets;
					}
					rets.put(Conts.KEY_RET_STATUS, CRSStatusEnum.STATUS_FAILED_DS_ZT_BANKCARD_AUTHEN_EXCEPTION);
					rets.put(Conts.KEY_RET_TAG, new String[] { Conts.TAG_UNFOUND });
