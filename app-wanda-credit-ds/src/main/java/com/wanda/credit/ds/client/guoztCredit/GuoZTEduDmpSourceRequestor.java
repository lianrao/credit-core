...
@DataSourceClass(bindingDataSourceId="ds_dmp_gztedu")
public class GuoZTEduDmpSourceRequestor extends BaseGuoZTDataSourcesRequestor implements IDataSourceRequestor {
	private final Logger logger = LoggerFactory.getLogger(GuoZTEduDmpSourceRequestor.class);
    @Autowired
    ...
    public Map<String, Object> request(String trade_id, DataSource ds) {
        ...
        String eduUrl = propertyEngine.readById("ds_dmp_eduGztUrl");
        ...
        try {
            ...
            if (inCache) {
                ...
                if(inCachPY){
                    ...
                    logger.info("{}数据中存在此人学历数据!", new String[] { prefix});
                    ...
                    if ("0".equals(personBase.getCollege())) {
                        ...
                        //-------------计算学历评估模型--------------------------
                        riskInfo = baseIncaheGZTService.EduRiskInfo(personBase.getGraduateYears(), gender+"", college.getIs211(), degree.getStudyType(), personBase.getDegree());
            ...
            if (!inCache) {
                ...
                String url = buildRequestUrl(params, eduUrl, prefix);
                ...
                DataSourceLogEngineUtil.writeLog2LogSys(new LoggingEvent(trade_id, String.valueOf(rspDataMap.get("rspstr")), new String[] { trade_id }));

                JSONObject data = rspData.getJSONArray("data").getJSONObject(0);

                logObj.setBiz_code3(rspData.getString("seq"));
                ...
                JSONObject checkInfo = data.getJSONObject("CheckInfo");
                if(checkInfo == null){
                    logger.error("{} 国政通学历查询返回异常！", prefix);
                    rets.clear();
                    rets.put(Conts.KEY_RET_STATUS, CRSStatusEnum.STATUS_FAILED_DS_EDU_EXCEPTION);
                ...
                JSONObject message = checkInfo.getJSONObject("message");
                if(message == null){
                    logger.error("{} 国政通学历查询返回异常！", prefix);
                    rets.clear();
                    rets.put(Conts.KEY_RET_STATUS, CRSStatusEnum.STATUS_FAILED_DS_EDU_EXCEPTION);
                ...
                if(BIZ_CODE_1_SUCCESS.equals(status)){
                    ...
                    degrees.setEnrolDate(checkInfo.getString("enrolDate"));
                    degrees.setIs211(checkInfo.getString("is211"));
                    degrees.setEducationDegree(checkInfo.getString("educationDegree"));
                    degrees.setIs985(checkInfo.getString("is985"));
                    degrees.setDstudyStyle(checkInfo.getString("dstudyStyle"));
                    degrees.setSchoolType(checkInfo.getString("schoolType"));
                    degrees.setBshldzNum(checkInfo.getString("bshldzNum"));
                    degrees.setCreateDate(checkInfo.getString("createDate"));
                    degrees.setEducationApproach(checkInfo.getString("educationApproach"));
                    degrees.setGraduate(checkInfo.getString("graduate"));
                    degrees.setStudyStyle(checkInfo.getString("studyStyle"));
                    degrees.setSsdNum(checkInfo.getString("ssdNum"));
                    degrees.setSpecialityName(checkInfo.getString("specialityName"));
                    degrees.setSchoolCategory(checkInfo.getString("schoolCategory"));
                    degrees.setZdxkNum(checkInfo.getString("zdxkNum"));
                    degrees.setBsdNum(checkInfo.getString("bsdNum"));
                    degrees.setSchoolTrade(checkInfo.getString("schoolTrade"));
                    degrees.setSchoolNature(checkInfo.getString("schoolNature"));
                    degrees.setAcademicianNum(checkInfo.getString("academicianNum"));
                    degrees.setCreateYear(checkInfo.getString("createYear"));
                    degrees.setUserName(checkInfo.getString("userName"));
                    degrees.setLevel(checkInfo.getString("level"));
                    degrees.setGraduateTime(checkInfo.getString("graduateTime"));
                    degrees.setOrganization(checkInfo.getString("organization"));
                    degrees.setSchoolCity(checkInfo.getString("schoolCity"));
                    degrees.setStudyResult(checkInfo.getString("studyResult"));
                    degrees.setBirthday(birthDays);
                    degreesService.save(degrees);
//                    degrees.setCardNo(cardNo);
                    ...
                    riskInfo = baseIncaheGZTService.EduRiskInfo(gradeYears, gender+"", college.getIs211(), degrees.getStudyStyle(), degrees.getEducationDegree());
                    if("100".equals(riskInfo)){
                        riskInfo = "";
                    }
                    personBase.setRiskAndAdviceInfo("");
                    ...
                    logger.info("{} 国政通学历查询失败:{}", new String[] { prefix, JSON.toJSONString(message)});
                    rets.clear();
                    rets.put(Conts.KEY_RET_STATUS, CRSStatusEnum.STATUS_FAILED_DS_EDU_EXCEPTION);
                    rets.put(Conts.KEY_RET_MSG, "学历查询失败!");
                    rets.put(Conts.KEY_RET_TAG, new String[]{initTag});
    ...
    public String buildRequestUrl(String params, String url, String prefix) {
        StringBuilder sb = new StringBuilder();
        sb.append(url).append("?");
        sb.append("queryParam=").append(params);

        logger.info("{} 请求数据源地址： {}", prefix, sb.toString());
        return sb.toString();
    }
