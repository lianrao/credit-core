...
@DataSourceClass(bindingDataSourceId="ds_xiaohe_bankCard4")
public class XiaoHeBankFourRequestor extends BasePSGDataSourceRequestor implements IDataSourceRequestor {
	private Logger logger = LoggerFactory.getLogger(XiaoHeBankFourRequestor.class);
	...
	public Map<String, Object> request(String trade_id, DataSource ds) {
		...
		logger.info("{} 小河-银行卡四要素调用开始..." , prefix);
		...
		String police_api = propertyEngine.readById("ds_xiaohe_bank4_api");
		...
		try{
			...
			String phone = ParamUtil.findValue(ds.getParams_in(), paramIds[3]).toString();//账户号
			...
			+paramMap.get("idname")+paramMap.get("phoneNo")+police_api_key+sdate;
			...
			logger.info("{} 小河-银行卡四要素出参包装开始..." , prefix);
			if("200".equals(code)){
		...
		logger.info("{} 银行卡四要素完成，交易时间为(ms):{}",prefix ,(System.currentTimeMillis() - start));
		return rets;
	}
}
