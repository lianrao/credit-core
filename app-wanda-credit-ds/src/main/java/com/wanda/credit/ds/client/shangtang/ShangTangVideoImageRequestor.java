...
@DataSourceClass(bindingDataSourceId = "ds_shangtang_contrast02")
public class ShangTangVideoImageRequestor extends BaseDataSourceRequestor
		implements IDataSourceRequestor {
	private final Logger logger = LoggerFactory
			.getLogger(ShangTangVideoImageRequestor.class);
	@Autowired
	...
	public Map<String, Object> request(String trade_id, DataSource ds) {
		...
		try {
			...
			byte[] req_video = (byte[]) ParamUtil.findValue(ds.getParams_in(),
					paramIds[0]); // 视频
			byte[] req_image = (byte[]) ParamUtil.findValue(ds.getParams_in(),
					paramIds[1]); // 水印照
			...
			String videoEncode = encoder.encode(req_video);  
			String imageEncode = encoder.encode(req_image);
			long size = videoEncode.length();
			logger.error("{} 视频大小:{} MB", prefix,size/(1024*1024));
			ImageUtils.decodeBase64ToImage(videoEncode, imgPath, trade_id
					+ ".mp4");
			ImageUtils.decodeBase64ToImage(imageEncode, imgPath, trade_id
					+ ".jpg");
			if (StringUtils.isNotEmpty(videoEncode)) {
				String videopath = null;
				logger.info("{} 视频上传征信存储开始...", prefix);
				videopath = fileService.upload(videoEncode, FileType.JPG,
			...
			if (StringUtils.isNotEmpty(imageEncode)) {
				String pathimage = null;
				logger.info("{} 图片上传征信存储开始...", prefix);
				pathimage = fileService.upload(imageEncode, FileType.JPG,
						FileArea.DS, trade_id);
			...
			if (Integer.parseInt(respMap.getString("code")) > 1000) {
				...
				if ("false".equals(respMap.getString("passed").toString())) {
					...
					rets.put(Conts.KEY_RET_MSG,
							"活体检测未通过 !");
					rets.put(Conts.KEY_RET_TAG, new String[] { resource_tag });
					logger.info("{} 活体检测未通过：{}", prefix, respMap);
				}else {
					...
					if (StringUtils.isNotEmpty(base64_image)) {
						...
						logger.info("{} 图片上传征信存储成功,照片ID为：{}", prefix, base64_image);
					}
					videophoto.setPassed(respMap.getString("passed"));
					videophoto.setLiveness_score(respMap
							.getString("liveness_score").toString());
					...
					if ("0".equals(return_image)) {
						respResult.put("base64_image", respMap.get("base64_image"));
					}else{
						respResult.put("base64_image","");
					}
					respResult.put("liveness_score",(Float.parseFloat(respMap
							.get("liveness_score").toString())) * 100);
					respResult.put("verification_score", (Float.parseFloat(respMap
							.get("verification_score").toString())) * 100);
	...
	public String httpClientPost(String vopath, String imagepath,
			String prefix) throws ClientProtocolException,
			IOException {
