...
@DataSourceClass(bindingDataSourceId="ds_tianchuang_detailBank4")
public class TCBankCheck4DetailRequestor extends BaseTianChSourceRequestor
		implements IDataSourceRequestor {
	private final  Logger logger = LoggerFactory.getLogger(TCBankCheck4DetailRequestor.class);
	@Autowired
	...
	public Map<String, Object> request(String trade_id, DataSource ds){
		...
		String url = propertyEngine.readById("ds_tianchuang_detailbank4_url");
		...
		try{	
			...
			String phone = String.valueOf(ParamUtil.findValue(ds.getParams_in(), paramIds[3])); // 照片数据包 	
			...
			logger.error("{} 天创信用银行卡四要素详情版查询结束:{}", prefix,res);
			...
			if("0".equals(json.getString("status"))){
				...
				logObj.setState_msg(data.getString("detailMsg"));
				logObj.setBiz_code1(data.getString("result"));
				logObj.setBiz_code2(json.getString("seqNum"));
				
				resource_tag = detailBulidResp(trade_id,data,retdata,rets);
				...
				logger.info("{} 天创信用银行卡四要素详情版传入参数有误", prefix);
				...
				logger.info("{} 天创信用银行卡四要素详情版验证失败1", prefix);
				rets.put(Conts.KEY_RET_STATUS, CRSStatusEnum.STATUS_FAILED_DS_ZT_BANKCARD_AUTHEN_EXCEPTION);
				rets.put(Conts.KEY_RET_MSG, "银行卡鉴权失败");
				rets.put(Conts.KEY_RET_TAG, new String[] { resource_tag });
				logger.error("{} 银行卡鉴权失败,返回结果为空", trade_id);
