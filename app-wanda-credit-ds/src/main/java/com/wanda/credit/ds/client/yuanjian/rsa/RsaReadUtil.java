package com.wanda.credit.ds.client.yuanjian.rsa;
...
//		InputStream inps=null;
		try {
			priKeyStream = RsaReadUtil.class.getResourceAsStream(pfxPath);//"/depends/ds/xyan/xxxfile.pxg"
//			priKeyStream = new FileInputStream(pfxPath);
			...
			return getPrivateKeyByStream(reads, priKeyPass);
			 ...
			 logger.error("解析文件，读取私钥失败:{}", ExceptionUtil.getTrace(e));
		} finally {
	...
	public static PrivateKey getPrivateKeyByStream(byte[] pfxBytes, String priKeyPass) {
		try {
			...
			logger.error("私钥存储异常:{}", ExceptionUtil.getTrace(e));
		} catch (NoSuchAlgorithmException e) {
			logger.error("不存在的解密算法:{}", ExceptionUtil.getTrace(e));
		} catch (CertificateException e) {
			logger.error("证书异常:{}", ExceptionUtil.getTrace(e));
		} catch (UnrecoverableKeyException e) {
			logger.error("不可恢复的秘钥异常:{}", ExceptionUtil.getTrace(e));
		}
		return null;
	}
}
