 ...
 *
 */
@DataSourceClass(bindingDataSourceId="ds_jxlMobileRawDataByDetail")
public class JXLMobileRawDataByDetailRequestor extends BaseDataSourceRequestor implements
		IDataSourceRequestor {
	
	
	private final  Logger logger = LoggerFactory.getLogger(JXLMobileRawDataByDetailRequestor.class);
	private String[] paramIds;
	@Resource(name="ds_jxlMobileRawData")
	private IDataSourceRequestor jxlMobileRawDataReq;
	@Autowired
	...
	public Map<String, Object> request(String trade_id, DataSource ds) {
		...
		logger.info("{} 准备根据用户身份证-姓名-手机号获取最新的通话详单，tradeId=" + trade_id ,prefix);
...
//		String encMobileNo = CryptUtil.encrypt(mobileNo);
		...
		trade.setName(name);
		trade.setTrade_flag(JXLConst.TF_GET_MOBILE_RAWDATA_BY_DET);
		...
		try{
			...
			List<ApplyBasicInfoPojo> basicList = jxlBasicInfoService.queryNewMobileCollectInPeriod(name, encCardNo, encMobileNo, 1);
			...
			if (basicList != null && basicList.size() > 0) {
				...
				if (StringUtil.areNotEmpty(requestId)) {
					...
					rets = jxlMobileRawDataReq.request(trade_id, reportDs); 
				}else{
					...
					rets.put(Conts.KEY_RET_MSG, "没有对应的通话详单数据，可能未提交采集请求或采集请求未完成");
					
					trade.setRet_code(CRSStatusEnum.STATUS_FAILED_DS_JUXINLI_NO_REPORTDATA.getRet_sub_code());
				}
			}else{
			...
			logger.error("{} 根据身份证，姓名，手机号获取通话详单数据异常 " + e.getMessage() ,prefix);
			rets.clear();
			rets.put(Conts.KEY_RET_STATUS, CRSStatusEnum.STATUS_FAILED);
			rets.put(Conts.KEY_RET_MSG, "获取通话详单数据失败");
		...
		try{
			...
			logger.error("{} 保存交易数据失败，交易序列号为" + trade_id + "异常信息为" + e.getMessage() ,prefix);
		}
		
		logger.info("{} 准备根据用户身份证-姓名-手机号获取最新的通话详单 END，tradeId=" + trade_id ,prefix);
		long tradeTime = System.currentTimeMillis() - startTime;
		logger.info("{} 根据身份证号，姓名，手机号获取最新通话详单总共耗时时间为（ms）" + tradeTime ,prefix);
