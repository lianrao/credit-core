...
import com.wanda.credit.base.util.AESUtils;
import com.wanda.credit.base.util.CardNoValidator;
import com.wanda.credit.base.util.ExceptionUtil;
import com.wanda.credit.base.util.RSAUtils;
...
@DataSourceClass(bindingDataSourceId="ds_rsa_educationQuery")
public class RSAEducationDataSourceRequestor extends BasePengYuanNewSourceRequestor implements IDataSourceRequestor {
	
	private final  Logger logger = LoggerFactory.getLogger(RSAEducationDataSourceRequestor.class);
	...
	public Map<String, Object> request(String trade_id, DataSource ds) {
		...
		logObj.setDs_id("ds_rsa_educationQuery");
		logObj.setReq_time(new Timestamp(System.currentTimeMillis()));
		logObj.setReq_url(propertyEngine.readById("sys.credit.client.rsa.pengyuan.url"));
		logObj.setIncache("0");	
		...
		try {
			...
			if(!inCached){
				...
				//解析返回报文
				Document rspDocNew = DocumentHelper.parseText(respXML);
				Node status = rspDocNew.selectSingleNode("//result/status");
				...
				if(status!=null && !"1".equals(status.getStringValue())){
					...
					logObj.setBiz_code3(trade_id); 
					Node errMsg = rspDocNew.selectSingleNode("//result/errorMessage");
					if(errMsg != null){
					...
					omitFields(stream);
					//校验开始...
					Node rtnode = rspDoc.selectSingleNode("//*/lastEducationEncryptInfo");
					if(rtnode!=null){
					...
					// encryptInfo解析
					node = rspDoc.selectSingleNode("//*/encryptInfo");
					String encryptInfo = node.getStringValue();
					/**对加密字符串进行解密操作**/
					node = rspDoc.selectSingleNode("//*/keyinfo/key");
					String encryptAESKey = node.getStringValue();
					node = rspDoc.selectSingleNode("//*/keyinfo/vkey");
					String vkey = node.getStringValue();
					// 1、先根据RSA密钥版本号获取对应的RSA私钥
					String privateKey = getRSAPrivateKey(vkey);
					// 2、再用根据RSA私钥解密加密后的AES密钥
					String decryptAESKey = RSAUtils.decryptByPrivateKey(encryptAESKey, privateKey);
					logger.info("解密后的AES秘钥 {}", decryptAESKey);
					// 3、最后用解密后的AES密钥解密加密后的学历信息
					String decryptEducationInfo = AESUtils.decryptWithKeyBase64(encryptInfo, decryptAESKey);
					logger.info("解密后的学历信息 {}", decryptEducationInfo);
					
					// 将String转document文件
					Document respEducationInfo = DocumentHelper.parseText(decryptEducationInfo);
					// degreeInfo和collegeInfo
					node = respEducationInfo.selectSingleNode("//educationInfo");
					if(node!=null){
						Node nd = node.selectSingleNode("degreeInfo");
						...
						if(degree!=null){
							...
							personBase.setDegree(degree.getDegree());
							personBase.setCollege(degree.getCollege());
							personBase.setGraduateTime(degree.getGraduateTime());
							personBase.setGraduateYears(EndTime(degree.getGraduateTime()));
						...
						nd = node.selectSingleNode("collegeInfo");
						stream.alias("collegeInfo", Py_edu_college.class);
						college = (Py_edu_college) stream.fromXML(nd.asXML());
					}
					// 将密钥版本号存在datasource_log_new中的BIZ_CODE2字段中
					logObj.setBiz_code2(vkey);
...
//			重新拼装degree
			...
			degreeNew.setStudyType(degree.getStudyType());
			...
			if ("0".equals(personBase.getCollege())) {
				...
				if(personBase != null && personBase.getDegree() != null){
...
*/		
	...
	private WebServiceSingleQueryOfUnzipSoapBindingStub createStubEdu(String trade_id) 
		...
		// 修改请求路径为访问加密数据的路径
		String url = propertyEngine.readById("sys.credit.client.rsa.pengyuan.url");
...
//	添加拼装学历类别的方法mafei-add
   ...
   private boolean isTimeoutException(Exception ex){
	...
	/**
	 * 根据数据源返回的版本号获取对应的RSA私钥
	 * @param vkey
	 * @return
	 */
	public String getRSAPrivateKey (String vkey) {
		if ("1.0.0".equals(vkey)) {
			return propertyEngine.readById("sys.credit.client.pengyuan.1.0.0.key");
		} else {
			logger.info("没有找到版本号对应的RSA私钥!");
		}
		return "";
	}
