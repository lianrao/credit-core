...
@DataSourceClass(bindingDataSourceId="ds_qh_blacklist_glad")
public class BlackListDataSourceRequestor_2_0 extends BaseQHDataSourceRequestor implements IDataSourceRequestor {
	private final Logger logger = LoggerFactory.getLogger(BlackListDataSourceRequestor_2_0.class);
	...
	public Map<String, Object> request(String trade_id, DataSource ds) {
		...
		try {
			...
			String ips = (String)ParamUtil.findValue(ds.getParams_in(), "ips"); // ip集
			String mobiles = (String)ParamUtil.findValue(ds.getParams_in(), "mobiles"); // 手机号码集
			String cardIds = (String)ParamUtil.findValue(ds.getParams_in(), "cardIds"); // 银行卡号集

			String cryptedMobiles = synchExecutorService.encrypt(mobiles);
			String cryptedCardIds = synchExecutorService.encrypt(cardIds);
			...
			context.put("ips", ips);
			context.put("moblieNos", mobiles);
			context.put("cardNos", cardIds);
			context.put("cryptedIdNo", cryptedIdNo);
			context.put("cryptedMobiles", cryptedMobiles);
			context.put("cryptedCardIds", cryptedCardIds);
			...
			Map<String, Object> param = CommonUtil.sliceMap(context, new String[]{NAME,IDNO,"ips","entityAuthCode","entityAuthDate"});
			param.put("moblieNos", context.get("cryptedMobiles"));
			param.put("cardNos", context.get("cryptedCardIds"));
	...
	protected Map<String, Object> buildRequestBody(Map<String, Object> data) {
		...
		// 目前只支持单条查询
		record.put("seqNo", 1);
		record.putAll(CommonUtil.sliceMapIfNotBlank(data, new String[]{"ips","moblieNos","cardNos"}));
		list.add(record);
