/**   
* @Description: 数据源-启信宝-根据人名和企业名称获得投资信息
* @author liunan
...
@DataSourceClass(bindingDataSourceId="ds_qxb_enternameCorp")
public class QXBNameCorpRequestor extends BaseQXBDataSourceRequestor
		implements IDataSourceRequestor {	
	private final static Logger logger = LoggerFactory.getLogger(QXBNameCorpRequestor.class);
	
	private final static String RETDATA_DETAIL = "detail";
	@Autowired
	public IPropertyEngine propertyEngine;
	@Autowired
	private QXBNameCorpHisRequestor hisService;
	public Map<String, Object> request(String trade_id, DataSource ds) {		
		String prefix = trade_id + " " + Conts.KEY_SYS_AGENT_HEADER;
		// 记录交易开始时间
		long startTime = System.currentTimeMillis();
		logger.info("{} 启信宝-根据企业全名和自然人名获取该自然人任法定代表人,对外投资及任职信息", prefix);
		// 组织返回对象
		...
		try{
			...
			//获取配置参数
			String reqUrl = propertyEngine.readById("qxb_enter_nameCorp_url");
			...
			if(statusInt==200){
				...
				ds.setId("ds_qxb_historynameCorp");
				Map<String,Object> hisrets = hisService.request(trade_id, ds);
				String hisstatus = String.valueOf(hisrets.get("retstatus"));
				if("STATUS_SUCCESS".equals(hisstatus)){
					Map<String,Object> hisret = (Map<String, Object>) hisrets.get("retdata");
					retData.put("history", hisret.get("detail"));
				}else{
					Map<String,Object> history = new HashMap<String,Object>();
					List<Map<String,Object>> list = new ArrayList<Map<String,Object>>();
					history.put("oper_info", list);
					history.put("invest_info", list);
					history.put("manager_info", list);
					retData.put("history", history);
				}
