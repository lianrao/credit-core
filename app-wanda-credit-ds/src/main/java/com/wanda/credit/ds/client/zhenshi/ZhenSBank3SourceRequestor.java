 ...
 * 描述: 银行卡三要素验证
 */
@DataSourceClass(bindingDataSourceId = "ds_zhenshi_bank3")
public class ZhenSBank3SourceRequestor extends BaseZhenShiRequestor
		implements IDataSourceRequestor {
	private final Logger logger = LoggerFactory.getLogger(ZhenSBank3SourceRequestor.class);
	@Autowired
	...
	public Map<String, Object> request(String trade_id, DataSource ds) {
		...
		logger.info("{} 甄视银行卡三要素请求开始...", prefix);
		TreeMap<String, Object> retdata = new TreeMap<String, Object>();
		Map<String, Object> rets = null;
		DataSourceLogVO logObj = new DataSourceLogVO(trade_id);
		Map<String, Object> reqparam = new HashMap<String, Object>();
		logObj.setReq_time(new Timestamp(System.currentTimeMillis()));// log请求时间
		String zhenshi_url = propertyEngine.readById("ds_zhenshi_bank3_url");// 调用连接
		String zhenshi_bank_api = propertyEngine.readById("ds_zhenshi_api");// api
		String zhenshi_bank_pwd = propertyEngine.readById("ds_zhenshi_api_pwd");// id
		...
		try {
			...
			logger.info("{} 甄视银行卡三要素服务加密成功!", prefix);
			reqparam.put("name", name);
			reqparam.put("cardNo", cardNo);
			reqparam.put("cardId", cardId);
			logObj.setReq_url(zhenshi_url);
			...
			if (StringUtils.isNotEmpty(CardNoValidator.validate(cardNo))) {
				...
				logObj.setIncache("0");
				logger.info("{} 甄视银行卡三要素请求开始...", prefix);
				String res = doZhensPost(zhenshi_url,getNameIDCardAccountVerifyParas(name,cardNo,cardId
						,zhenshi_bank_api,zhenshi_bank_pwd),"utf-8");
				logger.info("{} 甄视银行卡三要素请求结束,返回信息:{}", prefix, res);
				...
				String status = resJson.getString("RESULT");
				logObj.setBiz_code1(resJson.getString("guid"));
				if("1".equals(status)){
					logger.info("{} 甄视银行卡三要素请求成功", prefix);
	...
	public static String getNameIDCardAccountVerifyParas(String name, String idCard, String bankcard
		...
		totalJsonObj.addProperty("serviceName", "NameIDCardAccountVerify");

		JsonObject paramJsonObj = new JsonObject();
		paramJsonObj.addProperty("idCard", idCard);
		paramJsonObj.addProperty("name", name);
		paramJsonObj.addProperty("accountNo", bankcard);
