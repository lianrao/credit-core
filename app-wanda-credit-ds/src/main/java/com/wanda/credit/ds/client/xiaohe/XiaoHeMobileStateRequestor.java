 ...
 * 小河在网状态
 * */
@DataSourceClass(bindingDataSourceId="ds_xiaohe_mobileState")
public class XiaoHeMobileStateRequestor extends BasePPXDSRequestor implements IDataSourceRequestor {
	private Logger logger = LoggerFactory.getLogger(XiaoHeMobileStateRequestor.class);
	...
	public Map<String, Object> request(String trade_id, DataSource ds) {
		...
		String mobile_api = propertyEngine.readById("ds_xiaohe_mobilestate_api");
		...
		String onlineOfcode = propertyEngine.readById("ds_xiaohe_status_code");
		//初始化对象
		...
		try{
			...
			logger.info("{} 小河实时手机在网状态开始...", prefix);
        	Date now = new Date(); 
        	...
        	logger.info("{} 小河实时手机在网状态调用成功:{}", prefix,httpresult);
			...
			if("200".equals(code)){
				...
				result.put("mobileState", getOnlineCode(onlineOfcode,online_code));
