 ...
 * 信雅达在网时长
 * */
@DataSourceClass(bindingDataSourceId="ds_xinyada_mobileOnline")
public class XinYDMobileOnlineRequestor extends BasePPXDSRequestor implements IDataSourceRequestor {
	private Logger logger = LoggerFactory.getLogger(XinYDMobileOnlineRequestor.class);
	...
	public Map<String, Object> request(String trade_id, DataSource ds) {
		...
		logger.info("{} 信雅达实时手机在网时长开始...", prefix);
		...
		String onlineOfcode = propertyEngine.readById("ds_xinyada_online_code");
		//初始化对象
		...
		try{
        	...
        	logger.info("{} 信雅达实时手机在网时长调用成功:{}", prefix,httpresult);
			...
			if("0000".equals(xinydBean.getCode())){
				...
				result.put("inTime", getOnlineCode(onlineOfcode,xinydBean.getData().getValue()));
			}else if("1009".equals(xinydBean.getCode()) || "1401".equals(xinydBean.getCode())){
				result.put("inTime", "5");
			}else{
				result.put("inTime", "-1");
			}			
