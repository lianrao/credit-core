 ...
 * 描述: 人脸比对
 */
@DataSourceClass(bindingDataSourceId = "ds_linkface_liveCheck")
public class LinkPhotoCheckSourceRequestor extends BaseLinkFaceRequestor
		implements IDataSourceRequestor {
	private final Logger logger = LoggerFactory.getLogger(LinkPhotoCheckSourceRequestor.class);
	@Autowired
	...
	public Map<String, Object> request(String trade_id, DataSource ds) {
		...
		logger.info("{} linkface银行卡三要素请求开始...", prefix);
		...
		String link_bank3_api = propertyEngine.readById("ds_linkface_bank3_api");// api
		...
		String bank3_url = link_url+link_bank3_api;
		...
		try {
			...
			logger.info("{} linkface银行卡三要素服务加密成功!", prefix);
			reqparam.put("name", name);
			reqparam.put("cardNo", cardNo);
			reqparam.put("cardId", cardId);
			logObj.setReq_url(bank3_url);
			...
			if (StringUtils.isNotEmpty(CardNoValidator.validate(cardNo))) {
				...
				logger.info("{} linkface银行卡三要素请求开始...", prefix);
				 String res = RequestHelper.doPost(bank3_url,null,headers,params,null,false, 10000);
				logger.info("{} linkface银行卡三要素请求结束,返回信息:{}", prefix, res);
				...
				if("OK".equals(status)){
					logger.info("{} linkface银行卡三要素请求成功", prefix);
