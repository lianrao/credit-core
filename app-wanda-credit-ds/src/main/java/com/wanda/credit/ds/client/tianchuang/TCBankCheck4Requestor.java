...
@DataSourceClass(bindingDataSourceId="ds_tianchuang_bank4")
public class TCBankCheck4Requestor extends BaseTianChSourceRequestor
		implements IDataSourceRequestor {
	private final  Logger logger = LoggerFactory.getLogger(TCBankCheck4Requestor.class);
	@Autowired
	...
	public Map<String, Object> request(String trade_id, DataSource ds){
		...
		String url = propertyEngine.readById("ds_tianchuang_bank4_url");
		...
		try{	
			...
			if(StringUtil.isEmpty(res)){
				logger.error("{} 天创信用银行卡四要素查询返回异常！", prefix);
				rets.clear();
			...
			logger.error("{} 天创信用银行卡四要素查询结束:{}", prefix,res);
			...
			if("0".equals(json.getString("status"))){
				...
				logger.info("{} 天创信用银行卡四要素传入参数有误", prefix);
				...
				logger.info("{} 天创信用银行卡四要素验证失败1", prefix);
				rets.put(Conts.KEY_RET_STATUS, CRSStatusEnum.STATUS_FAILED_DS_ZT_BANKCARD_AUTHEN_EXCEPTION);
				rets.put(Conts.KEY_RET_MSG, "银行卡鉴权失败");
				rets.put(Conts.KEY_RET_TAG, new String[] { resource_tag });
				logger.error("{} 银行卡鉴权失败,返回结果为空", trade_id);
