...
@DataSourceClass(bindingDataSourceId="ds_yitu_auth")
public class YiTuAuthDataSourceRequestor extends BaseYiTuDataSourceRequestor implements IDataSourceRequestor {
	private final Logger logger = LoggerFactory.getLogger(YiTuAuthDataSourceRequestor.class);
	...
	public Map<String, Object> request(String trade_id, DataSource ds) {
		...
		logObj.setDs_id("ds_yitu_auth");//log:供应商id
...
//			paramIn.put("photo", photo);
			...
			if(StringUtils.isNotEmpty(photo)){
				...
				fpath = fileService.upload(photo, FileType.JPG, FileArea.DS,trade_id);
				logger.info("{} 图片上传征信存储成功,照片ID为：{}", prefix,fpath);
			}
			if(formatStrSize(photo)){
				...
				params.put("query_image_content", photo);
