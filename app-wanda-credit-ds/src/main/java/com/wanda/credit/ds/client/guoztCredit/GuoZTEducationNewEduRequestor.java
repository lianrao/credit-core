...
@DataSourceClass(bindingDataSourceId="ds_guozt_eduNew02")
public class GuoZTEducationNewEduRequestor extends BaseGuoZTDataSourcesRequestor implements IDataSourceRequestor {
	private final Logger logger = LoggerFactory.getLogger(GuoZTEducationNewSourceRequestor.class);
	@Autowired
	private BaseGuoZTPPdRequestor baseIncaheGZTService;
	@Autowired
		...
		try {
			...
			if (inCache) {
				...
				if(inCachPY){
					...
					logger.info("{}存在此人学历数据!", new String[] { prefix});
					...
					if ("0".equals(personBase.getCollege())) {
						...
						if(personBase != null){
						  topDegreeCode = BaseGuoZTPPdRequestor.degreeMap.get(personBase.getDegree());
					...
					rets = baseIncaheGZTService.getEduIncacheGuoZT(mapStr,date_num);
					if(rets == null){
						inCache = false;
					}else{
...
//				logger.info("{}国政通学历数据源返回数据：{}", new String[] { prefix,respXml });
//				String respXml = IOUtils.toString(new FileInputStream("e:/temp/gztxl1.xml"));
...
//						personBase.setVerifyResult(1);
						...
						if(BaseGuoZTPPdRequestor.degreeMap.get(personBase.getDegree())!=null){
							topDegreeCode = BaseGuoZTPPdRequestor.degreeMap.get(personBase.getDegree());
							topDegree = personBase.getDegree();
						}
						...
						logObj.setState_code(DataSourceLogEngineUtil.TRADE_STATE_SUCC);
						logObj.setBiz_code1(BaseGuoZTPPdRequestor.checkCode.get(status));
