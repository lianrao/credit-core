...
@DataSourceClass(bindingDataSourceId="ds_zsPersonQuery")
public class ZSPersonInfoDataSourceRequestor  extends BaseZSDataSourceRequestor 
			implements IDataSourceRequestor {
	private final  Logger logger = LoggerFactory.getLogger(ZSPersonInfoDataSourceRequestor.class);
	...
	public Map<String, Object> request(String trade_id, DataSource ds) {
		...
		logObj.setDs_id("ds_zsPersonQuery");//log:供应商id
		...
		try {
			...
			String cardNo = ParamUtil.findValue(ds.getParams_in(), paramIds[0]).toString(); //身份证号	
			...
			String xmlValue = toXmlStr(userId,ChinaDaasDataEncoder.encode(cardNo,"V1"),keyType,orders,userPwd);
			// des加密
			...
			if(errCodeNode!=null && StringUtils.isNotEmpty(errCodeNode.getText())){
				...
				enRegiCode = ChinaDaasDataEncoder.encode(cardNo,"V1");
				String oracle_code = enRegiCode.replaceAll("<n>", "＜n＞");
				Map<String,Object> getResultMap = orderService.inCachedProd(oracle_code,enRegiCode,acct_id);
				if("1".equals(getResultMap.get("STAT"))){
					resource_tag = Conts.TAG_FOUND_OLDRECORDS;
				}else{
					resource_tag = Conts.TAG_FOUND_NEWRECORDS;
				}
				Map<String,Object> getResultDsMap = orderService.inCachedDsNew(oracle_code,enRegiCode);
					...
					if ("1".equals(order.getSTATUS())) {
						...
						if(list!=null){
							...
							for(Element e : list){
								...
								JSONObject jsonObject = JSONObject.fromObject(punishbreak);
								jsonArray.add(jsonObject);
							}
							retdata.put("punishbreaks",  punishbreaks!=null ? om.writeValueAsString(punishbreaks) : null);
							retdata.put("punishbreaks_json",  jsonArray!=null ? jsonArray : null);
						}
						//个人被执行信息
						...
						if(list!=null){
							...
							for(Element e : list){
								...
								JSONObject jsonObject = JSONObject.fromObject(punished);
								jsonArray.add(jsonObject);
							}
							retdata.put("punisheds",  punisheds!=null ? om.writeValueAsString(punisheds) : null);
							retdata.put("punisheds_json",  jsonArray!=null ? jsonArray : null);
						}
