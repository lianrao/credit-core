...
@DataSourceClass(bindingDataSourceId="ds_tianchuang_detailBank3")
public class TCBankCheck3DetailRequestor extends BaseTianChSourceRequestor
		implements IDataSourceRequestor {
	private final  Logger logger = LoggerFactory.getLogger(TCBankCheck3DetailRequestor.class);
	@Autowired
	...
	public Map<String, Object> request(String trade_id, DataSource ds){
		...
		String url = propertyEngine.readById("ds_tianchuang_detailbank3_url");
		...
		try{	
			...
			if(StringUtil.isEmpty(res)){
				logger.error("{} 天创信用银行卡三要素详情版查询返回异常！", prefix);
				rets.clear();
			...
			logger.error("{} 天创信用银行卡三要素详情版查询结束:{}", prefix,res);
			...
			if("0".equals(json.getString("status"))){
				...
				logger.info("{} 天创信用银行卡三要素详情版验证失败1", prefix);
				rets.put(Conts.KEY_RET_STATUS, CRSStatusEnum.STATUS_FAILED_DS_ZT_BANKCARD_AUTHEN_EXCEPTION);
				rets.put(Conts.KEY_RET_MSG, "银行卡鉴权失败");
				rets.put(Conts.KEY_RET_TAG, new String[] { resource_tag });
				logger.error("{} 银行卡鉴权失败,返回结果为空", trade_id);
