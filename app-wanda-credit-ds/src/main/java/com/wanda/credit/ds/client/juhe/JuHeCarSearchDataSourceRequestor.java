...
/**
 * 330	车辆信息查询V2
 * （error_code=0或233004计费）
 * @author liunan
 */
@DataSourceClass(bindingDataSourceId="ds_juhe_carSearch")
public class JuHeCarSearchDataSourceRequestor extends BaseJuheDSRequestor
implements IDataSourceRequestor {
	private final  Logger logger = LoggerFactory.getLogger(JuHeCarSearchDataSourceRequestor.class);
	@Autowired
	...
	public Map<String, Object> request(String trade_id, DataSource ds){
		...
		try{
			String url = propertyEngine.readById("ds_juhe_verifyCarSearch_url");
			int incache_days = Integer.valueOf(propertyEngine.readById("ds_juhe_carSearch_incacheTime"));//车辆数据时间(天)
			...
			String carNumber = ParamUtil.findValue(ds.getParams_in(), "carNumber").toString(); //车牌号
			String carType = ParamUtil.findValue(ds.getParams_in(), "carType").toString(); //车辆类型:02
 
            reqparam.put("key", propertyEngine.readById("ds_juhe_verifyCarSearch_key"));
			...
			if(juheSaveCarService.inCachedCount(name, carNumber,ds.getId(), incache_days)){
				logger.info("{} 聚合车辆信息数据库查询匹配到数据",trade_id);
				logObj.setState_code(DataSourceLogEngineUtil.TRADE_STATE_SUCC);
				String incache_map = juheSaveCarService.findCarDetail(ds.getId(), name, carNumber, incache_days);
			...
			mapStr.put("key", propertyEngine.readById("ds_juhe_verifyCarSearch_key"));
			mapStr.put("name", name);
			mapStr.put("number", carNumber);
			mapStr.put("type",carType);
			logObj.setIncache("0");
			...
			if("0".equals(rspData.getString("error_code")) || "233004".equals(rspData.getString("error_code"))){
				...
				rets.put(Conts.KEY_RET_MSG, "采集成功!");
				rets.put(Conts.KEY_RET_TAG, new String[]{resource_tag});
				juheSaveCarService.saveCarDetail(trade_id, ds.getId(), name, carNumber, JSON.toJSONString(result));
				logger.info("处理成功" , prefix);
			}else if("233001".equals(rspData.getString("error_code"))){
				logObj.setState_code(DataSourceLogEngineUtil.TRADE_STATE_SUCC);
				logger.info("{} 传入参数有误:{}" , prefix,rspData.getString("reason"));
				resource_tag = Conts.TAG_SYS_ERROR;
				rets.put(Conts.KEY_RET_STATUS, CRSStatusEnum.STATUS_FAILED_SYS_PARAM_INVALID);
				rets.put(Conts.KEY_RET_MSG, rspData.getString("reason"));
				rets.put(Conts.KEY_RET_TAG, new String[]{resource_tag});
				return rets;
			}else if("233003".equals(rspData.getString("error_code"))){
				logObj.setState_code(DataSourceLogEngineUtil.TRADE_STATE_SUCC);
				...
				rets.put(Conts.KEY_RET_MSG, "查无车辆信息");
				rets.put(Conts.KEY_RET_TAG, new String[]{resource_tag});
				return rets;
			}else{
				resource_tag = Conts.TAG_SYS_ERROR;
