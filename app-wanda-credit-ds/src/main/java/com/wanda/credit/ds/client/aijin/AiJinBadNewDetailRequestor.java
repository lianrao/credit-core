 ...
 * 爰金不良信息 101400
 * */
@DataSourceClass(bindingDataSourceId="ds_aijin_new_badMsg")
public class AiJinBadNewDetailRequestor extends BaseAijinDataSourceRequestor
		implements IDataSourceRequestor {
	private final  Logger logger = LoggerFactory.getLogger(AiJinBadNewDetailRequestor.class);
	@Autowired
	...
	public Map<String, Object> request(String trade_id, DataSource ds){
		...
		int incache_days = Integer.valueOf(propertyEngine.readById("ds_yuanjin_badmsg_incache_days"));//服务代码
		logger.info("{}爱金不良信息数据源:{} {} {}", prefix,yuanjin_face_account,yuanjin_face_acode,yuanjin_face_key);
		String enCardNo = "";
		...
		try{	
			...
			if (badInfoService.inCachedCount(name, enCardNo, ds.getId(), incache_days)) {
				logObj.setIncache("1");
				logObj.setState_code(DataSourceLogEngineUtil.TRADE_STATE_SUCC);
				String badInfoList = badInfoService.findCertiDetail(ds.getId(), name, enCardNo, incache_days);
				resource_tag = Conts.TAG_INCACHE_TST_SUCCESS;
				retdata.putAll(JSONObject.parseObject(badInfoList));
	        ...
	        logger.info("{}爱金不良信息请求结束:{}", prefix,json);
	        ...
	        if("100".equals(bad_msg.getResponseCode())){
	        	...
	        	if(!"1".equals(bad_msg.getResult()) || StringUtil.isEmpty(bad_msg.getResultData())){
	        		...
	        		if(badMsg.contains("REVOKE")){
	        			resource_tag = Conts.TAG_TST_SUCCESS;
	        			retdata.put("details", bad_msg.getResultData().getBadDetail());
						retdata.put("num", bad_msg.getResultData().getBadCnt());
						retdata.put("date", bad_msg.getResultData().getNewestDate());
	        		}else{
	        			resource_tag = Conts.TAG_TST_SUCCESS;
						...
						retdata.put("num", bad_msg.getResultData().getBadCnt());
						retdata.put("date", bad_msg.getResultData().getNewestDate());
						badInfoService.saveCertiResult(trade_id, ds.getId(), name, enCardNo, JSONObject.toJSONString(retdata));
	        		}
	        	}
	        }else{
