...
@DataSourceClass(bindingDataSourceId="ds_juhe_AuthenBankCard4")
public class JuHeAuthenBankCard4DataSourceRequestor extends BaseJuHeAuthenBankCardDataSourceRequestor
implements IDataSourceRequestor {
	private final  Logger logger = LoggerFactory.getLogger(JuHeAuthenBankCard4DataSourceRequestor.class);
	...
	public Map<String, Object> request(String trade_id, DataSource ds){
		...
		try{			
			...
			logObj.setReq_url(propertyEngine.readById("wdwsh_jhauthcard4url"));
            ...
            String bankName = (String) ParamUtil.findValue(ds.getParams_in(),"bankName");
			...
			/*//查询缓存
			Map<String, Object> cacheData = getCache(prefix,name, cardNo, cardId, phone);
			if (cacheData != null) {
				logObj.setIncache("1");
			...
			String url = buildRequestUrl(trade_id,name,cardNo,cardId,phone);
...
//				juheAuthCardService.saveAuthCard4(trade_id,
//						name,cardNo,cardId,phone,rspData);
				...
				allAuthCardService.savaJuHeAuthCard(ds.getId(), trade_id, name,
						encCardNo, encCardId, encPhone, rspData.getJSONObject("data"), req_values);
	...
	protected Map<? extends String, ? extends Object> visitBusiData(
		...
		ret.put("respCode", product.get("result"));
		ret.put("respDesc", product.get("msg"));
		return ret;
	}*/
	
	public String buildRequestUrl(String trade_id, String name, String cardNo,
			String cardId,String phone) {
		String url = propertyEngine.readById("wdwsh_jhauthcard4url");
		StringBuilder sb = new StringBuilder();
		sb.append(url).append("?");		
		sb.append("idcard=").append(cardNo).append("&realname=").append(name).
		append("&mobile=").append(phone).append("&bankcard=").append(cardId);		
		return sb.toString();
	}
